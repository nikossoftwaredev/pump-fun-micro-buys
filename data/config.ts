export const headerLinks = [
  { path: `#about`, text: "About" },
  { path: `#usage`, text: "Usage" },
  { path: `#token`, text: "Token" },
  { path: `#socials`, text: "Socials" },
];

export const TWITTER_URL = "https://x.com/micropumpbot";
export const TELEGRAM_URL = "https://t.me/micropumpbot";
export const TELEGRAM_BOT_URL = "https://t.me/micropump_bot?start=6416185160";

export const GRADIENT_STYLE =
  "bg-gradient-to-r from-solana-purple to-solana-green text-transparent text-4xl font-extrabold text-center bg-clip-text";

export const TOKEN_SYMBOL = "$MICRO";
export const TOKEN_NAME = "MicroPump";
export const TOTAL_SUPPLY = "1B";

export const TOKEN_ADDRESS = "8VsX66CN1V9j5cZvgxhRzm84evMqzv8FWd7Toxr4bonk";
export const RAYDIUM_URL = `https://raydium.io/swap/?inputCurrency=sol&outputCurrency=${TOKEN_ADDRESS}&fixed=in`;
export const BIRDEYE_URL = `https://birdeye.so/token/${TOKEN_ADDRESS}?chain=solana`;
export const DEX_URL = `https://dexscreener.com/solana/${TOKEN_ADDRESS}`;
export const DEX_TOOLS_URL = `https://www.dextools.io/app/en/solana/pair-explorer/${TOKEN_ADDRESS}?t=1711125507735`;
export const PUMP_URL = `https://www.pump.fun/${TOKEN_ADDRESS}`;
export const BONK_URL = `https://letsbonk.fun/token/comingsoon`;

export const SERVER_URL = "http://34.171.163.77:8080";
